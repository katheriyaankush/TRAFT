...
export default class UIElementActions {
  ...
  public async getAttribute(attributeName: string): Promise<string> {
    ...
    await this.waitTillVisible();
    const text = await this.getLocator().getAttribute(attributeName)
    // Fix the string | null issue
    if (text == null) {
      return "";
    }
  ...
  * @returns
  */
  public async isEditable(sec: number = CommonConstants.DEFAULT_TIMEOUT): Promise<boolean> {
    Log.info(`Checking if ${this.description} is editable`);
    const element = this.getLocator();
    return await element.isEditable({ timeout : sec });
  }
   ...
   */
  public async isEnabled(sec: number = CommonConstants.DEFAULT_TIMEOUT): Promise<boolean> {
    Log.info(`Checking if ${this.description} is enabled`);
    const element = this.getLocator();
    return await element.isEnabled({ timeout: sec });
  }
   ...
   * @returns Promise<boolean>
   */
  public async isVisible(sec: number = CommonConstants.DEFAULT_TIMEOUT): Promise<boolean> {
    let visibility: boolean;
    Log.info(`Checking if ${this.description} is visible`);
    try {
      await this.getLocator().waitFor({ state: 'visible' , timeout: sec});
      visibility = await this.getLocator().isVisible({ timeout: sec });
    } catch (error) {
      visibility = false;
      Log.info(`${this.description} is not visible even after ${sec} seconds`);
    }
    return visibility;
  }
  ...
  public async mouseClick() {
    ...
    // Fix the null issue
    if (box == null) {
      throw new Error(`Element ${this.description} is not visible`);
    }
